# type: ignore
"""
机票 Agent：基于 LangGraph ReAct + MCP 工具，仅负责机票预订。
...
class AirTicketingAgent(BaseAgent):
    """机票预订 Agent：LangGraph ReAct + MCP 工具，工具首次使用时从 MCP 服务器加载。"""
    ...
    def __init__(self):
        ...
        self.instructions = prompts.AIRFARE_COT_INSTRUCTIONS
        self._runtime = JSONMCPToolRuntime(
            instructions=self.instructions,
            tool_specs=[
                ToolSpec(
                    name="search_flights",
                    description="Search flights by origin, destination, ticket class, and optional result limit.",
                    required_args=("origin", "destination"),
                    optional_args=("ticket_class", "limit"),
                ),
                ToolSpec(
                    name="get_travel_inventory",
                    description="Inspect supported airport codes and ticket classes.",
                ),
                ToolSpec(
    ...
    def get_agent_response(self, chunk: str | dict) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(data, str):
                # 仅当能解析出 dict 时才作为 data artifact，否则作为 text 返回（DataPart 要求 dict）
                try:
            ...
            # 其它类型（如 list）统一按 text 返回，避免 DataPart 校验失败
