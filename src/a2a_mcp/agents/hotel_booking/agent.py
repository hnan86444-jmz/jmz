# type: ignore
"""
酒店预订 Agent：基于 LangGraph ReAct + MCP 工具，仅负责酒店预订。
...
class HotelBookingAgent(BaseAgent):
    """酒店预订 Agent：LangGraph ReAct + MCP 工具，工具首次使用时从 MCP 服务器加载。"""
    ...
    def __init__(self):
        ...
        self.instructions = prompts.HOTELS_COT_INSTRUCTIONS
        self._runtime = JSONMCPToolRuntime(
            instructions=self.instructions,
            tool_specs=[
                ToolSpec(
                    name="search_hotels",
                    description="Search hotels by city, hotel type, room type, and optional result limit.",
                    required_args=("city",),
                    optional_args=("hotel_type", "room_type", "limit"),
                ),
                ToolSpec(
                    name="get_travel_inventory",
                    description="Inspect supported hotel cities, hotel types, and room types.",
                ),
                ToolSpec(
